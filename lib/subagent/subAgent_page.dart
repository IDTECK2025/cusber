...
class SubAgentManagementScreen extends StatefulWidget {
  @override
  _SubAgentManagementScreenState createState() =>
      _SubAgentManagementScreenState();
}
...
class _SubAgentManagementScreenState extends State<SubAgentManagementScreen> {
  ...
  Future<void> fetchSubAgents() async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        List<SubAgent> allSubAgents = [];
        for (int i = 0; i < userData.length; i++) {
          try {
            var userJson = userData[i];
            print('Parsing user $i: $userJson');
            allSubAgents.add(SubAgent.fromJson(userJson));
          } catch (e) {
        ...
        setState(() {
          subAgents =
              allSubAgents
                  .where((user) => user.role.toLowerCase() == 'subagent')
                  .toList();
          isLoading = false;
        });
  ...
  List<SubAgent> get filteredSubAgents {
    if (searchQuery.isEmpty) {
      return subAgents;
    }

    return subAgents.where((subAgent) {
      return subAgent.name.toLowerCase().contains(searchQuery.toLowerCase()) ||
          (subAgent.email?.toLowerCase().contains(searchQuery.toLowerCase()) ??
              false) ||
          (subAgent.phone?.contains(searchQuery) ?? false);
    }).toList();
  }

  List<SubAgent> get paginatedSubAgents {
    final filtered = filteredSubAgents;
  ...
  int get totalItems => filteredSubAgents.length;
  ...
  Widget _buildHeader(bool isDesktop, bool isTablet, bool isMobile) {
    return Row(
      ...
      children: [
        ...
        RefreshButton(isDesktop, isTablet, onTap: fetchSubAgents),
      ],
    );
  }
    ...
    return Container(
      ...
      child: Column(
        children: [
          ...
          Expanded(
            child:
                paginatedSubAgents.isEmpty
                    ? _buildEmptyState()
                    : ListView.builder(
                      itemCount: paginatedSubAgents.length,
                      itemBuilder: (context, index) {
                        final subAgent = paginatedSubAgents[index];
                        final isExpanded = expandedSubAgentId == subAgent.id;
  ...
  Widget _buildSubAgentRow(
    ...
    final isExpanded = expandedSubAgentId == subAgentId;
    final holderId = subAgent.id;
          ...
          children: [
            ...
            Expanded(
              ...
              child: Text(
                ...
                style: TextStyle(
                  color: subAgent.active ? Colors.green[700] : Colors.red[700],
                  fontSize: 12,
                  fontWeight: FontWeight.w500,
                ),
              ),
  ...
  Widget _ExpandedSubAgentDetails(
    SubAgent subAgent,
    bool isDesktop,
    bool isTablet,
    bool isTabletMini,
      ...
      child: Column(
        ...
        children: [
          Row(
            ...
            mainAxisAlignment: MainAxisAlignment.center,
            children: [
              Avatar(name: subAgent.name, size: 40),
              SizedBox(width: 16),
              Expanded(
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
          ...
          SizedBox(height: 16),
          _buildInfoWrap(subAgent, isDesktop, isTablet, isTabletMini, isMobile),
        ],
      ),
    );
  }
  ...
  Widget _buildInfoWrap(
    ...
    return Wrap(
      ...
      children: [
        _buildInfoItem("Address", subAgent.address ?? 'N/A'),
  ...
  void _toggleExpansion(String subAgentId) {
    setState(() {
      if (expandedSubAgentId == subAgentId) {
        expandedSubAgentId = null;
      } else {
        expandedSubAgentId = subAgentId;
      }
    });
  }
    ...
    return Container(
      ...
      child: Row(
        children: [
          Expanded(
            flex: 3,
            child: Text('Sub Agent', style: _headerTextStyle()),
          ),
          if (isDesktop)
